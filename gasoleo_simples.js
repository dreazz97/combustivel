...
//Sesimbra = 230
  ...
  async function getFuelData() {
    ...
    if (myKeysValues == '?municipio=222'){
      municipio = 222;
      document.querySelector(".title").innerHTML="Preço Gasóleo Simples Mais Barato de Almada.";
      document.getElementById("municipio").selectedIndex = 0
    } else if (myKeysValues == '?municipio=229') {
      municipio = 229;
      document.querySelector(".title").innerHTML="Preço Gasóleo Simples Mais Barato do Seixal.";
      document.getElementById("municipio").selectedIndex = 1
    } else if (myKeysValues == '?municipio=230') {
    municipio = 230;
    document.querySelector(".title").innerHTML="Preço Gasóleo Simples Mais Barato de Sesimbra.";
    document.getElementById("municipio").selectedIndex = 2
    } else if(myKeysValues == '') {
      document.getElementById("municipio").selectedIndex = 1
    }
    var seixal_fuel_response = await fetch(`https://precoscombustiveis.dgeg.gov.pt/api/PrecoComb/PesquisarPostos?idsTiposComb=2101&idMarca=&idTipoPosto=&idDistrito=&idsMunicipios=${municipio}&qtdPorPagina=50&pagina=1`);
...
var click_gasoleo_simples = document.querySelector('#gasoleo_aditivado');
